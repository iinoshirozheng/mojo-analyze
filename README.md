...
- a Category D decomposition series progressively moved the bottleneck upstream: the [parse+hash experiment](experiments/2026-09-15-csvagg-parsehash-decomposition.md) showed the arm64 Mojo/C gap survives removal of hash-table operations; the [parser-only follow-up](experiments/2026-09-16-csvagg-parseronly-decomposition.md) removed variable-length FNV and still measured about **1.91× on x86-64 and 3.57× on arm64**; the [scanner-only follow-up](experiments/2026-09-17-csvagg-scanonly-decomposition.md) removed numeric accumulation and still measured about **1.60×–1.63× on x86-64 and 3.72×–3.73× on arm64**; the [same-binary ingestion-vs-scan boundary](experiments/2026-09-18-csvagg-ingestion-scan-boundary.md) moved only file loading outside the timer. On that run the x86-64 ratio moved modestly from **1.694× to 1.613×**, while arm64 collapsed from **4.134× to 2.350×**. File materialization is therefore a major arm64 contributor, but a substantial in-memory scanner/codegen gap remains. A [Span-vs-Pointer access probe](experiments/2026-09-19-csvagg-span-pointer-scan.md) then held scanner semantics and the checksum constant while changing only byte access: unchecked pointer reads cut Mojo scanner mean time by **23.9% on x86-64** and **40.9% on arm64**, reducing the same-run Mojo/C ratio from **1.409× to 1.072×** and **1.854× to 1.097×**, respectively. Optimized assembly showed that the checked `Span` path retains inlined bounds guards and diagnostic slow paths, making checked byte access a material cause of the residual Mojo 1.0.0 scanner gap. A [Mojo 1.0→1.1 differential](experiments/2026-09-20-csvagg-span-version-differential.md) repeated the same mechanism probe and found the checked path essentially unchanged in Mojo 1.1: pointer mode remained about **24% faster on x86-64 and 42% faster on arm64**. A [Mojo 1.1→1.2 nightly differential](experiments/2026-09-21-csvagg-span-mojo12-nightly.md) again found no material fix: the x86-64 pointer control stayed essentially flat while remaining about **25% faster** than checked `Span`; on arm64 the stable/median checked path was essentially unchanged and pointer mode remained about **42% faster**. The arm64 nightly run was bimodal; an [independent exact rerun](experiments/2026-09-22-csvagg-span-arm64-rerun.md) reproduced the same fast and slow timing modes while their occupancy flipped from **9/6 to 4/11**. Stable controls and an almost unchanged fast-mode center strengthen the evidence that the multimodal distribution itself is real for this pinned compiler/source/runner combination, but do not identify its cause or justify calling it a compiler regression. An [arm64 CPU-affinity follow-up](experiments/2026-09-23-csvagg-span-arm64-affinity.md) then pinned the same Mojo 1.2 checked-`Span` binary to CPU 0 and still observed both regimes: **14 fast / 6 slow** when unpinned and **12 fast / 8 slow** when pinned, while the pointer and C controls remained tightly clustered. Preventing migration among the runner's four logical CPUs is therefore insufficient to explain the multimodality; the root cause remains open. Because mode occupancy can move the median between populations, mean/median alone are not treated as sufficient summaries for this result. These are mechanism probes, not recommendations to make the canonical benchmark unsafe. Canonical results remain unchanged.
