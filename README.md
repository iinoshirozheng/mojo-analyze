...
- a Category D decomposition series progressively moved the bottleneck upstream: the [parse+hash experiment](experiments/2026-09-15-csvagg-parsehash-decomposition.md) showed the arm64 Mojo/C gap survives removal of hash-table operations; the [parser-only follow-up](experiments/2026-09-16-csvagg-parseronly-decomposition.md) removed variable-length FNV and still measured about **1.91× on x86-64 and 3.57× on arm64**; then the [scanner-only follow-up](experiments/2026-09-17-csvagg-scanonly-decomposition.md) removed quantity/price numeric accumulation as well and still measured about **1.60×–1.63× on x86-64 and 3.72×–3.73× on arm64**. The severe arm64 disadvantage therefore already exists before numeric parsing, narrowing the next investigation to the file-ingestion / byte-scanning / loop-control boundary. Canonical results remain unchanged.
