...
- a [Category D parse+hash decomposition](experiments/2026-09-15-csvagg-parsehash-decomposition.md) first showed that the arm64 Mojo/C gap survives removal of hash-table operations. A [parser-only follow-up](experiments/2026-09-16-csvagg-parseronly-decomposition.md) then removed variable-length FNV as well: the same-run Mojo/C parser-only ratio was about **1.91× on x86-64 and 3.57× on arm64**. The severe arm64 disadvantage therefore precedes FNV and is now narrowed to the file-read / delimiter-scanner / numeric-parser prefix rather than a portable hash or hash-table bottleneck. Canonical results remain unchanged.
