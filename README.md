...
- a Category D decomposition series progressively moved the bottleneck upstream: the [parse+hash experiment](experiments/2026-09-15-csvagg-parsehash-decomposition.md) showed the arm64 Mojo/C gap survives removal of hash-table operations; the [parser-only follow-up](experiments/2026-09-16-csvagg-parseronly-decomposition.md) removed variable-length FNV and still measured about **1.91× on x86-64 and 3.57× on arm64**; the [scanner-only follow-up](experiments/2026-09-17-csvagg-scanonly-decomposition.md) removed numeric accumulation and still measured about **1.60×–1.63× on x86-64 and 3.72×–3.73× on arm64**; finally the [same-binary ingestion-vs-scan boundary](experiments/2026-09-18-csvagg-ingestion-scan-boundary.md) moved only file loading outside the timer. On that run the x86-64 ratio moved modestly from **1.694× to 1.613×**, while arm64 collapsed from **4.134× to 2.350×**. File materialization is therefore a major arm64 contributor, but a substantial in-memory scanner/codegen gap remains. Canonical results remain unchanged.
