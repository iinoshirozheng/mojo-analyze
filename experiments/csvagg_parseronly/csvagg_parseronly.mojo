...
def main() raises:
    ...
    # Preserve the canonical Category-D scanner/parser prefix through all four fields.
    var i = 0
    ...
    while i < n:
        # Field 0: order_id — scan only, matching canonical behavior.
        while i < n and data[i] != COMMA:
            i += 1
        i += 1

        # Field 1: category — retain span length as observable output, but do not hash it.
        var cat_start = i
        while i < n and data[i] != COMMA:
            i += 1
        var cat_end = i
        ...
        # Field 3: price_cents and end-of-line handling.
        var price_cents = 0
