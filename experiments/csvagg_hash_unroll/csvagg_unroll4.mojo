...
def main() raises:
    ...
    # Keep the canonical Category-D parser, table representation, probe
    # sequence, equality and aggregation unchanged. This experiment changes
    # only the variable-length FNV byte loop below.
    ...
    while i < n:
        ...
        # Focused intervention: unroll four dependent FNV steps per loop
        # iteration while keeping the algorithm/order exactly identical.
        while k + 4 <= cat_end:
            h = h ^ UInt64(data[k])
            h = h * FNV_PRIME
            h = h ^ UInt64(data[k + 1])
