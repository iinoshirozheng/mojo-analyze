"""Focused Category-C experiment: Rust default HashMap hasher vs FNV-1a.

Runs two otherwise-identical Rust word-frequency binaries, enforces exact
...
def run(binary: Path, warmup: int, trials: int):
    times = []
    checksums = set()
    cmd = [str(binary), "--corpus", str(CORPUS)]
...
def main():
    ...
    default_times, default_checksum = run(
        ROOT / "rust/target/release/wordfreq", args.warmup, args.trials
    )
    fnv_times, fnv_checksum = run(
        ROOT / "rust/target/release/wordfreq_fnv", args.warmup, args.trials
    )
    if default_checksum != fnv_checksum:
        raise RuntimeError(
            f"checksum mismatch: default={default_checksum} fnv={fnv_checksum}"
        )

    default = summarize(default_times)
    fnv = summarize(fnv_times)
    ratio = default["mean"] / fnv["mean"]

    payload = {
        "experiment": "wordfreq-rust-default-vs-fnv1a",
        "arch": args.arch,
        "warmup": args.warmup,
    ...
    out = OUT_DIR / f"wordfreq-rust-fnv-{args.arch}.json"
    out.write_text(json.dumps(payload, indent=2) + "\n")

    print(f"checksum: {default_checksum}")
