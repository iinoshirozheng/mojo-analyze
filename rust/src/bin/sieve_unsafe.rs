...
fn main() {
    ...
    while i * i <= limit {
        if is_prime[i] {
            let mut j = i * i;
            while j <= limit {
                // Experiment: `j <= limit` and the vector length is
                // `limit + 1`, so the index is proven in-bounds by the
                // surrounding loop. Skip only this hot marking-write bounds
                // check; keep every other operation identical to the safe
                // baseline so the experiment isolates this one cost.
                unsafe {
